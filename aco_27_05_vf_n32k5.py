...
Original file is located at
    https://colab.research.google.com/drive/1r4aS6i8Eg60inQNoCXqAIljDGd4ECdH3
"""
...
demandes_df = pd.read_excel("/content/demandes 32 clients merg.xlsx", index_col="date")

# Lire les coordonnées (en supposant que la colonne "client" contient "entrepot", "CL1", etc.)
coordonnees_df= pd.read_excel("/content/les coordonnées des 32 clients et l'entrepot.xlsx", sheet_name="Feuil1",index_col="client ")
...
time_windows_df = pd.read_excel("fenetres_temps 32.xlsx", index_col="Client")
...
service_times_df = pd.read_excel("temps_service 32.xlsx", index_col="Client")
...
class ACOVRP:
    def __init__(self, dist_matrix, demandes, params, time_windows, service_times, vehicules_data):
        ...
        self.vehicules_data = vehicules_data
    ...
    def run(self):
                ...
                for _ in range(self.params["iterations"]):
                    ...
                    for solution in solutions:
                        ...
                        clients_servis = set()
                        for route, _, distance in solution:  # Changé à 3 éléments
                            clients_servis.update(route[1:-1])

                        if len(clients_servis) == self.n_clients:
                            total_distance = sum(d for _, _, d in solution)  # Changé à 3 éléments
                ...
                if meilleure_solution is None:
                    meilleure_solution = self.generer_solution_garantie()  # Assurez-vous que cette fonction gère les contraintes de temps
                    # Correction: la solution de secours ne contient que 3 éléments
    ...
    # Génère des solutions complètes pour chaque fourmi en utilisant 6 véhicules
...
vehicules_data = {
        0: {"type": "essence", "emissions": 150, "age": 5}, # Exemple: véhicule 0 a 5 ans
...
def objective(trial):
    ...
    params = {
        "n_vehicules": trial.suggest_int("n_vehicules", 1, 5),
        "n_fourmis": trial.suggest_int("n_fourmis", 10, 50),
    ...
    aco = ACOVRP(dist_matrix, demandes, params, time_windows, service_times, vehicules_data)
...
def visualiser_route(solution, coordonnees_df):
    ...
    clients = [f"CL{i}" for i in range(1, 33)]
...
def visualiser_route_reelle(solution, coordonnees_df):
    ...
    couleurs_vehicules = ['#FF0000', '#0000FF', '#00FF00', '#FFA500', '#800080', '#A52A2A']
...
demandes_df = pd.read_excel("/content/demandes 32 clients merg.xlsx", index_col="date")
coordonnees_df = pd.read_excel("/content/les coordonnées des 32 clients et l'entrepot.xlsx", index_col="client ")

# Préparation des données
clients_ordered = [f"CL{i}" for i in range(1, 33)]
coords = coordonnees_df.loc[["entrepot"] + clients_ordered][["X", "Y"]].values
dist_matrix = distance_matrix(coords, coords)
date_cible = demandes_df.index[0]
demandes = [0] + [demandes_df.loc[date_cible, f"CL{i}"] for i in range(1, 33)]
...
aco = ACOVRP(dist_matrix, demandes, final_params, time_windows, service_times, vehicules_data) # Inclure time_windows et service_times
